...
class TimeWeightedVectorStoreRetriever(BaseRetriever):
    ...
    # embodied_memory_stream: List[Document] = Field(default_factory=list)
    ...
    def add_documents(self, documents: List[Document], **kwargs: Any) -> List[str]:
        ...
        for i, doc in enumerate(dup_docs):
            ...
            doc.metadata["node_id"] = f"node_{len(self.memory_stream) + i}"
        self.memory_stream.extend(dup_docs)
        # print("22222222222222222222", self.memory_stream)
        # ***add return param dup_docs
        return self.vectorstore.add_documents(dup_docs, **kwargs), dup_docs
    
    # ***add embodied documents
    # def add_embodied_documents(self, documents: List[Document], **kwargs: Any) -> List[str]:
    #     """Add documents to vectorstore."""
    #     current_time = kwargs.get("current_time")
    #     if current_time is None:
    #         current_time = datetime.datetime.now()
    ...
    #         if "created_at" not in doc.metadata:
    #             doc.metadata["created_at"] = current_time
    #         doc.metadata["buffer_idx"] = len(self.embodied_memory_stream) + i
    #     self.embodied_memory_stream.extend(dup_docs)
    #     return self.vectorstore.add_documents(dup_docs, **kwargs), dup_docs
